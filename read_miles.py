...
class MilesSSPs:
    ...
    def get_lmd(self):
        print("Assuming wavelength array is default from MILES wepage")
        self.lmd = np.arange(3540.5, 7409.6+0.9, 0.9)

    def truncate_wavelengths(self,
                             lmd_min=None,
                             lmd_max=None):
        print('Assuming original and desired pixels are linear in wavelength')
        if lmd_min is None:
    ...
    def bin_pixels(self, pix_per_bin=1):
        if pix_per_bin==1:
            ...
            if r<0:
                ...
                lmd = self.lmd
                X = self.X
            self.lmd = np.mean(np.reshape(lmd, (-1, pix_per_bin)), 1)
            self.X = np.sum(np.reshape(X, (pix_per_bin, -1, p)), 0)
